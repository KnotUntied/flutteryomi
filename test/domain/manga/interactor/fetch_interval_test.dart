...
final _testTime = DateTime.parse("2020-01-01T00:00:00Z");
final _chapter = ChapterUtils.create().copyWith(
  dateFetch: _testTime,
  dateUpload: _testTime,
);

final _fetchInterval = FetchInterval(MockGetChaptersByMangaId());

Chapter _chapterWithTime(Chapter chapter, Duration duration) {
  final newTime = _testTime + duration;
...
void main() {
  test('returns default interval of 7 days when not enough distinct days', () {
    final chaptersWithUploadDate = IntRange(1, 50).map(
    ...
    final chaptersWithoutUploadDate = chaptersWithUploadDate.map(
      (i) => i.copyWith(
  ...
  test('returns interval of 7 days when multiple chapters in 2 days', () {
    final chapters = IntRange(1, 2).map(
          (i) => _chapterWithTime(_chapter, 1.days),
        ) +
        IntRange(1, 5).map(
  ...
  test('returns interval of 1 day when chapters are released every 1 day', () {
    final chapters = IntRange(1, 20).map(
  ...
  test('returns interval with floored value when interval is decimal', () {
    final chaptersWithUploadDate = IntRange(1, 5).map(
