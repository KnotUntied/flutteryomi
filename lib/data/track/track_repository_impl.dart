...
class TrackRepositoryImpl implements TrackRepository {
  ...
  Future<Track?> getTrackById(int id) =>
      db.mangaSyncDrift.getTrackById(id: id).getSingleOrNull();

  @override
  Future<List<Track>> getTracksByMangaId(int mangaId) =>
      db.mangaSyncDrift.getTracksByMangaId(mangaId: mangaId).get();
  ...
  @override
  Future<void> delete(int mangaId, int trackerId) => (db.delete(db.mangaSync)
        ..where(
          (it) => it.mangaId.equals(mangaId) & it.trackerId.equals(trackerId),
        ))
      .go();

  @override
  Future<void> insert(Track track) => db //
      .into(db.mangaSync)
      .insert(
  ...
  @override
  Future<void> insertAll(List<Track> tracks) => db.batch((batch) {
        batch.insertAll(
          db.mangaSync,
          tracks.map(
