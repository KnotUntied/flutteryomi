...
class SyncChaptersWithSource {
  ...
  Future<List<Chapter>> await_(
    ...
    //    .mapIndexed((i, sChapter) => ChapterUtils.create()
    //        .copyFromSChapter(sChapter)
    //        .copyWith(name: sChapter.name.sanitize(manga.title), mangaId: manga.id, sourceOrder: i),
    //    );
    ...
    //final removedChapters = dbChapters //
    //    .whereNot(
    //      (dbChapter) => sourceChapters.any((sourceChapter) => dbChapter.url == sourceChapter.url),
    //    );
    ...
    //  final chapterNumber = ChapterRecognition.parseChapterNumber(manga.title, chapter.name, chapter.chapterNumber);
    //  chapter = chapter.copyWith(chapterNumber: chapterNumber);

    //  final dbChapter = dbChapters.firstOrNullWhere((it) => it.url == chapter.url);

    //  if (dbChapter == null) {
    //    Chapter toAddChapter;
    //    if (chapter.dateUpload == 0) {
    //      final altDateUpload = (maxSeenUploadDate == 0) ? nowMillis : maxSeenUploadDate;
    //      toAddChapter = chapter.copyWith(dateUpload: altDateUpload);
    //    } else {
    ...
    //      );
    //      if (chapter.dateUpload != 0) {
    //        toChangeChapter = toChangeChapter.copyWith(dateUpload: chapter.dateUpload);
    //      }
    ...
    //    await updateManga.awaitUpdateFetchInterval(
    //      manga,
    //      now,
    //      fetchWindow
    //    );
    ...
    //  var chapter = toAddItem.copyWith(dateFetch: DateTime(nowMillis + itemCount--));
    ...
    //  final chapterUpdates = updatedChapters.map((it) => it.toChapterUpdate()).toList();
    ...
    //return updatedToAdd.whereNot((it) => reAddedUrls.contains(it.url) || excludedScanlators.contains(it.scanlator)).toList();
    return [];
  }
}

@riverpod
SyncChaptersWithSource syncChaptersWithSource(SyncChaptersWithSourceRef ref) => SyncChaptersWithSource(
